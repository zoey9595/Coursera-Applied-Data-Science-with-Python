...
def answer_three():
    df['dif2'] = (df['Gold'] - df['Gold.1'])/(df['Gold'] + df['Gold.1'] + df['Gold.2'])
    selected_country = df[(df['Gold'] > 0) & (df['Gold.1'] > 0)]
    return selected_country.loc[selected_country['dif2'] == selected_country['dif2'].max()].index[0]
...
def answer_six():
    a = census_df[census_df['SUMLEV'] == 50].groupby('STNAME')['CENSUS2010POP'].apply(lambda x:x.nlargest(3).sum()).nlargest(3).index.values.tolist()
    return a
